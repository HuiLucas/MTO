...
MMA::MMA(PetscInt nn, PetscInt mm, PetscInt kk, Vec xo1t,Vec xo2t,Vec Ut,Vec Lt, 
	...
	VecDuplicate(xo1t, &L);//VecDuplicate(Vec old,Vec *new);To create a new vector of the same format as an existing vector, one uses the command
	VecDuplicate(xo1t, &U);
	...
	VecDuplicateVecs(xo1t,m, &pij);//VecDuplicateVecs(Vec old,PetscInt n,Vec **new);To create several new vectors of the same format as an existing vector, one uses the command
...
//×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*×*//
MMA::MMA(PetscInt nn, PetscInt mm,Vec x, PetscScalar *at, PetscScalar *ct,PetscScalar *dt,double init, double dec, double inc){
	n = nn;
	m = mm;

	asyminit = (PetscScalar)(init);
	asymdec = (PetscScalar)(dec);
	asyminc = (PetscScalar)(inc);

	NonLinConstraints = PETSC_TRUE;
	constraintModification = PETSC_TRUE;
	RobustAsymptotesType = 0;
...
PetscErrorCode MMA::Update(Vec xval, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax,PetscScalar raa0, double dec, double inc){
	PetscErrorCode ierr=0;
	asymdec = (PetscScalar)(dec);
	asyminc = (PetscScalar)(inc);
	if (!NonLinConstraints)	{
	...
	GenSub(xval,dfdx,gx,dgdx,xmin,xmax,raa0);
...
PetscErrorCode MMA::GenSub(Vec xval, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax, PetscScalar raa0){
	PetscErrorCode ierr=0;
	...
	PetscScalar raa0v = raa0/feps;
	for (PetscInt i=0; i<nloc; i++){
		alf[i] = Max(xminv[i],0.9*Lv[i]+0.1*xv[i]);
		bet[i]  = Min(xmaxv[i],0.9*Uv[i]+0.1*xv[i]);
		dfdxp = Max(0.0,dfdxv[i]);
		dfdxm = Max(0.0,-1.0*dfdxv[i]);
		p0v[i] = pow(Uv[i]-xv[i],2.0)*( dfdxp + 0.001*Abs(dfdxv[i]) + raa0v*feps);//Max(1.0e-5,xmaxv[i]-xminv[i]) );
		q0v[i] = pow(xv[i]-Lv[i],2.0)*( dfdxm + 0.001*Abs(dfdxv[i]) + raa0v*feps);//Max(1.0e-5,xmaxv[i]-xminv[i]) );
		for (PetscInt j=0;j<m;j++){
			dfdxp = Max(0.0,dgdxv[j][i]);
			dfdxm = Max(0.0,-1.0*dgdxv[j][i]);
			if (constraintModification){
				pijv[j][i] = pow(Uv[i]-xv[i],2.0)*( dfdxp + 0.001*Abs(dgdxv[j][i]) + raa0v*feps);///Max(1.0e-5,xmaxv[i]-xminv[i]) );
				qijv[j][i] = pow(xv[i]-Lv[i],2.0)*( dfdxm + 0.001*Abs(dgdxv[j][i]) + raa0v*feps);///Max(1.0e-5,xmaxv[i]-xminv[i]) );
			}else{
