"""
postprocess_schwarz_stl.py
==========================
Generate an STL of the optimised Schwarz-P heatsink lattice using LEAP71 /
PicoGK.  Reads the optimised RBF control-point displacements from
  schwarz_ctrl_pts_optimised.txt
and produces
  schwarz_optimised.stl
...
Run from anywhere:
    conda run -n LEAP71 python postprocess_schwarz_stl.py [--ctrl path/to/file.txt]
...
# ═══════════════════════════════════════════════════════════════════════════
# Optimisation / simulation domain  (must match schwarz_rbf_optimizer.py)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# Schwarz-P TPMS parameters  (must match schwarz_rbf_optimizer.py)
# ═══════════════════════════════════════════════════════════════════════════
F_UNIT_SIZE      = 1.5    # TPMS cell size (mm)
F_WALL_THICKNESS = 0.20   # wall thickness of the Schwarz-P solid shell (mm)
...
# ── RBF deformation field (identical to optimizer's RBFDeformationField) ──

class RBFDeformationField:
    """
    3D displacement field: RBF thin-plate spline → dense baked grid → trilinear lookup.
    Mirrors the implementation in schwarz_rbf_optimizer.py and the LEAP71 notebook.
    """

    def __init__(
        self,
        ctrl_pts:     np.ndarray,   # (N, 3) control-point positions (mm)
        disp_ctrl:    np.ndarray,   # (N, 3) displacement vectors (mm)
    ...
    def get_displacement(self, x: float, y: float, z: float) -> tuple:
        """Single-point trilinear lookup (called per voxel by DelegateImplicit)."""
