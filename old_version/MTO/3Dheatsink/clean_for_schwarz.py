"""
clean_for_schwarz.py — Reset the 3D heat-sink case for a fresh Schwarz RBF run.
...
Removes:
  ...
  • Cached Python optimizer files (cell_centers_mm.npy, schwarz_ctrl_pts*.txt,
    schwarz_opt_history.txt).
  • The app/0/gamma file written by the Schwarz optimizer (so gamma reverts to
...
Usage:
    python clean_for_schwarz.py [--case app] [--dry-run]
"""
...
REMOVE_OPTIMIZER_FILES = [
    ...
    'schwarz_opt_history.txt', 'cell_centers_mm.npy',
]
...
def clean_case(case_dir: Path, dry_run: bool) -> None:
    ...
    # Also remove app/0/gamma written by the Schwarz optimizer so the solver
    # falls back to the uniform initial value (voluse = 0.2).
    gamma0 = case_dir / '0' / 'gamma'
    if gamma0.exists():
        remove(gamma0, "Schwarz-written gamma in time 0")
    ...
    # Remove schwarz_ctrl_pts_*.txt files (wildcard)
    for p in case_dir.glob('schwarz_ctrl_pts*.txt'):
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Reset case for a fresh Schwarz RBF run.')
    ...
    print(f"=== clean_for_schwarz [{mode}]  case: {case_dir} ===")
