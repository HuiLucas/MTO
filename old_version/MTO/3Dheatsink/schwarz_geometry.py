"""
schwarz_geometry.py — standalone helper for generating and inspecting
the Schwarz-P gamma field on the 3D heat-sink OpenFOAM mesh.

Useful for:
  • Visualising what the Schwarz Primitive looks like before optimising.
...
Usage examples
--------------
# Generate initial (undeformed) Schwarz gamma and write to app/0/gamma
python schwarz_geometry.py --mode init --case app

# Load an existing VDB from the LEAP71 notebook and map it onto the mesh
python schwarz_geometry.py --mode vdb --vdb ../../LEAP71_version_KBE/Examples/ImplicitRandomSchwarzPrimitive_Python.vdb --case app

# Visualise the gamma field as a 2D slice plot (requires matplotlib)
python schwarz_geometry.py --mode plot --case app --time 10
"""
...
from schwarz_rbf_optimizer import (
    get_cell_centers_mm,
    write_gamma_field,
    read_scalar_field,
    schwarz_sdf_batch,
...
def gamma_from_vdb(
    ...
    The VDB stores the SDF of the Schwarz-P geometry:
...
# ── Initial analytic Schwarz gamma ────────────────────────────────────────────
...
def initial_gamma(
    ...
    """Undeformed Schwarz-P gamma (zero RBF displacement everywhere)."""
    zero_disp = np.zeros_like(cell_centers_mm)
    sdf   = schwarz_sdf_batch(cell_centers_mm, zero_disp, k, half_thickness)
...
def plot_gamma_slice(
    ...
    ax.set_title(f'Schwarz-P gamma  |  {axis.upper()}={target:.2f} mm slice')
    ax.set_aspect('equal')
    plt.tight_layout()
    if save_path:
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Schwarz geometry helper for the MTO case.')
    ...
    if args.mode in ('init', 'vdb'):
        ...
        if args.mode == 'init':
            ...
            print(f"  Undeformed Schwarz-P  solid_frac = {1 - gamma.mean():.4f}")
        else:
            if args.vdb is None:
