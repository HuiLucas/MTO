...
void tractionDisplacementFvPatchVectorField::updateCoeffs()
{
    ...
    const fvPatchField<scalar>& Po =
        patch().lookupPatchField<volScalarField, scalar>("Po");
    ...
    scalarField mu((E*eps+gamma*gamma*gamma*(1-eps)*E)/(2.0*(1.0 + Po)));
    scalarField lambda((E*eps+gamma*gamma*gamma*(1-eps)*E)*Po/((1.0 + Po)*(1.0 - 2.0*Po)));
